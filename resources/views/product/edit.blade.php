<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 form-body ">
            <nav class="bg-grey-light p-3 rounded font-sans w-full border-2 border-opacity-10 border-indigo-600">
                <ol class="list-reset flex text-grey-dark">
                    ...
                    <li><span class="mx-2">></span></li>
                    <li><a href="{{route('products.edit', $product->id)}}" class="text-black font-bold">{{ __('Edit') }} ({{$product->reference }} : {{ $product->name}})</a></li>
                </ol>
            </nav>
        </div>
    </x-slot>
    ...
    <x-slot name="slot">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 form-body ">
            <h1 class="mt-3" style="font-size: 32px; font-weight: bold"> {{ __('Product') }} Modification ({{$product->reference }} : {{ $product->name}}) </h1>
            <livewire:product-edit-form product="{{$product->id}}"/>
        </div>
    </x-slot>
</x-app-layout>
